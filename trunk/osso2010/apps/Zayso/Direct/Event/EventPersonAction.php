<?php
class Direct_Event_EventPersonAction extends Direct_BaseAction
{
...
EOT;
		...
		// Need event_id
		if (isset($params['event_id']) && $params['event_id']) $eventId = $params['event_id'];
		else
		{
			return $this->wrapResults(array());
		}
		$db   = $this->context->db;
		$rows = $db->fetchRows($sql,array('event_id' => $eventId));
