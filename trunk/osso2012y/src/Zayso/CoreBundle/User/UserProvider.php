...
class UserProvider implements UserProviderInterface
{
    ...
    public function __construct(EntityManager $em, $userClass, $masterPassword)
    {
        $this->em = $em;
...
EOT;
    ...
    /* =========================================================================
     * username could be username, aysoid, ussfid, email etc
     */
     ...
     */
    protected function getUserNameForOpenidIdentifier($identifier)
    {
        $sql = <<<EOT
   
SELECT account.user_name AS userName
...
EOT;
    ...
    protected function getUserNameForAysoid($aysoid)
    {
        $sql = <<<EOT
...
EOT;
        $db = $this->em->getConnection();
        $stmt = $db->prepare($sql);
        $params = array('aysoid' => 'AYSOV' . $aysoid);
        $stmt->execute($params);
        $row = $stmt->fetch(\PDO::FETCH_ASSOC);
