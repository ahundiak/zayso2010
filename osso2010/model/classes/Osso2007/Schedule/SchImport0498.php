<?php
class Osso2007_Schedule_SchImport0498 extends Cerad_Import
{
  protected $readerClassName = 'Osso2007_Schedule_SchReader0498';
  ...
  public function addEvent($date,$time,$fieldId,$homeTeamId,$awayTeamId)
  {
    // Need at least one team
    if (!$homeTeamId && !$awayTeamId) return 0;

    $result = $this->directSchTeam->fetchRow(array('sch_team_id' => $homeTeamId));
    if ($result->row) $homeTeam = $result->row;
    else              $homeTeam = NULL;

    $result = $this->directSchTeam->fetchRow(array('sch_team_id' => $awayTeamId));
    if ($result->row) $awayTeam = $result->row;
    else              $awayTeam = NULL;
    ...
    if ($result->rowCount)
    {
      ...
      $event = $result->rows[0];
      if ($event['team_id'] != $homeTeamId) die('event home team mismatch');
    }
    else
    {
      ...
      $this->directEventTeam->insert($data);
    }
    // Process away team
    ...
    if ($result->rowCount)
    {
      // Check that the team is not different
      $event = $result->rows[0];
      if ($event['team_id'] != $awayTeamId) die('event away team mismatch');
    }
    else
    {
  ...
  {   
    ...
    $homeTeamId = $this->processTeam($homeTeam);
    $awayTeamId = $this->processTeam($awayTeam);

    $this->addEvent($date,$time,$fieldId,$homeTeamId,$awayTeamId);
    
    // printf("Game %s %s %s %s %s\n",$date,$time,$fieldId,$homeTeamId,$awayTeamId);
  ...
  protected function processTeam($team)
  {
    // Check Cache
    if (!$team) return 0;
    if (isset($this->teamss[$team])) return $this->teams[$team];
    ...
    $schTeamId = $result->row['sch_team_id'];
    
    $this->teams[$team] = $schTeamId;
