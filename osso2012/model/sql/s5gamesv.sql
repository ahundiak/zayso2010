...
SELECT
  ...
  account.fname     AS account_fname,
...
-- 
DROP VIEW IF EXISTS vol_view;

CREATE VIEW         vol_view AS
SELECT
  vol.reg_num AS aysoid,
  vol.fname   AS fname,
  vol.lname   AS lname,
  vol.nname   AS nname,
  vol.dob     AS dob,
  ...
  cert_coach.typex      AS cert_coach_type,
  cert_referee.typex    AS cert_referee_type,
  cert_safe_haven.typex AS cert_safe_haven_type,

  cert_type_coach.desc1      AS cert_coach_desc1,
  cert_type_referee.desc1    AS cert_referee_desc1,
  cert_type_safe_haven.desc1 AS cert_safe_haven_desc1,

  reg_email.valuex  AS email,
  reg_phonec.valuex AS phonec,

  org.keyx  AS org_key,
  org.desc1 AS org_desc

FROM eayso.reg_main AS vol
...
-- ==================================================
DROP VIEW IF EXISTS account2007_view;

CREATE VIEW         account2007_view AS
SELECT
  account.account_id   AS id,
  account.account_user AS uname,
  account.account_pass AS upass,
